...
class ClassifyWater(QDialog, FORM_CLASS):
    ...
    def save_img_dialog(self):
        if self.new_img_status == "calibrated":
            ...
            filename_ending = "_land_highlighted"
        
        try:
            options = QFileDialog.Options()
            self.path_to_directory, _ = QFileDialog.getSaveFileName(self, "", self.path_to_file[:-4] + filename_ending, "PNG File (*.png) ;; JPEG File (*.jpeg) ;; PDF File (*.pdf)", options=options)
            self.save_img()
    ...
    def save_array_dialog(self):
        ...
        try:
            options = QFileDialog.Options()
            self.path_to_directory, file_type = QFileDialog.getSaveFileName(self, "", self.path_to_file[:-4] + filename_ending, "Numpy File (*.npy) ;; Text file (*.txt)", options=options)
    ...
    def set_rgb(self):
        if self.input_rgb.text() != "":
            if self.original_rgb_radiobutton.isChecked():
    ...
    def open_file(self):
        if self.path_to_file != "":
            original_img, _, _= self.get_img_from_hdr(self.path_to_file, rgb=self.original_rgb)
            self.show_img(original_img, self.label_original_img)


    def get_img_from_hdr(self, path_to_file, rgb):
        try:
            envi_img = envi.open(path_to_file)
    ...
    def perform_cube_calibration(self):
        try:
            ...
            cube_calibrated.astype(dtype = "uint16").tofile(self.path_to_file[:-4] + "_calibrated.bip")
            calibrated_hdr_path = shutil.copy2(self.path_to_file, self.path_to_file[:-4] + "_calibrated.hdr")
    ...
    def perform_atmospheric_correction(self):
        try:    
            ...
            self.cube_atmos_corrected.astype(dtype = "uint16").tofile(self.path_to_file[:-4] + "_atmospheric_corrected.bip")
            shutil.copy2(self.path_to_file, self.path_to_file[:-4] + "_atmospheric_corrected.hdr")
    ...
    def classify_water(self):
        try:
            if self.original_classify_radiobutton.isChecked():
                img = envi.open(self.path_to_file)

            elif self.calibrated_classify_radiobutton.isChecked():
                img = envi.open(self.path_to_file[:-4] + "_calibrated.hdr")
            
            wl = list(map(float, img.metadata["wavelengths"]))
            wl = wl[::-1] # needed to get corresponding order of spectra and wavelengths

            water_img_array = np.zeros((img.shape[0],img.shape[1]))
